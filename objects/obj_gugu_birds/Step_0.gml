...
else if grow_timer < grow_time+6*current_flash_speed{
	state = CARD_STATE.GROW
	grow_timer++
}
else if grow_timer == grow_time+6*current_flash_speed{
...
if timer < current_flash_speed - 1 {
    ...
    switch (state) {
        case CARD_STATE.IDLE:
            if (image_index >= flash_offset+1 && image_index < idle_anim+flash_offset) image_index++;
            else image_index = flash_offset+1;
            break;
            
        case CARD_STATE.ATTACK:
            if (image_index >= (idle_anim+1+flash_offset) && image_index < (idle_anim+attack_anim+flash_offset)) image_index++;
